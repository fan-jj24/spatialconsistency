...
def qwen3_5_decoder_layer_forward(
    ...
    **kwargs,
    ...
    # 兼容 transformers 5.14.1: decoder layer 实例上没有 layer_type 属性，
    # 从 config 的 layer_types 列表按 layer_idx 读取
    _layer_type = getattr(self, "layer_type", None)
    if _layer_type is None:
        ...
        if _cfg is not None:
            ...
            if _layer_types is None:
                ...
                _layer_types = getattr(_tc, "layer_types", None) if _tc else None
            if _layer_types:
                _layer_idx = getattr(self, "layer_idx", 0)
                _layer_type = _layer_types[_layer_idx]

    if _layer_type is None:
        raise AttributeError(
            f"Cannot resolve layer_type for layer {getattr(self, 'layer_idx', '?')}: "
            f"self has no 'layer_type' attr, and config has no 'layer_types'. "
            f"Check transformers version compatibility with verl qwen3_5.py"
        )

    if _layer_type == "linear_attention":
    ...
    elif _layer_type == "full_attention":
