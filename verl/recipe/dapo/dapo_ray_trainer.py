...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        num_gen_batches = 0
        _filter_total_groups = 0
        _filter_all_correct = 0
        _filter_all_wrong = 0
        _filter_kept = 0
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        # === 统计全对/全错/保留（累加） ===
                        _num_total = len(prompt_uid2metric_std)
                        _nc = 0
                        _nw = 0
                        for _uid, _std in prompt_uid2metric_std.items():
                            if _std == 0 and len(prompt_uid2metric_vals[_uid]) > 1:
                                _vals = prompt_uid2metric_vals[_uid]
                                if all(v > 0 for v in _vals):
                                    _nc += 1
                                elif all(v == 0 for v in _vals):
                                    _nw += 1
                        _filter_total_groups += _num_total
                        _filter_all_correct += _nc
                        _filter_all_wrong += _nw
                        _filter_kept += len(kept_prompt_uids)
                        # === 统计结束 ===
                ...
                metrics["filter/num_total_groups"] = _filter_total_groups
                metrics["filter/num_all_correct"] = _filter_all_correct
                metrics["filter/num_all_wrong"] = _filter_all_wrong
                metrics["filter/num_kept"] = _filter_kept
                metrics["filter/keep_ratio"] = _filter_kept / _filter_total_groups if _filter_total_groups > 0 else 0.0
                batch = None
                num_prompt_in_batch = 0
                num_gen_batches = 0
                _filter_total_groups = 0
