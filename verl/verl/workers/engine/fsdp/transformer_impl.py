...
class FSDPEngine(BaseEngine):
    ...
    def _build_module(self):
        ...
        # === FREEZE_VISION_PATCH: 冻结 vision tower 参数 ===
        _freeze_vision = os.environ.get("FREEZE_VISION", "0") == "1"
        if _freeze_vision:
            _frozen = 0
            _total = 0
            for name, param in module.named_parameters():
                _total += 1
                if any(kw in name.lower() for kw in ["vision", "visual", "image_encoder", "pixel"]):
                    param.requires_grad = False
                    _frozen += 1
            print(f"[FREEZE_VISION] 冻结 {_frozen}/{_total} 个参数（vision tower）")
        # === FREEZE_VISION_PATCH END ===
        return module
